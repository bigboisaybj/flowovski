  ...
  public function loadMerchants($data, $limit) {
    ...
    if(mysqli_num_rows($data_query) > 0) {
      ...
      while($row = mysqli_fetch_array($data_query)) {
        ...
        $merchantUser = array();
        array_push($merchantUser, $merchant_id);
        array_push($merchantUser, $userLoggedIn);
        ...
        $waitingTime = $this->checkWaitingTimeForUpdate($merchant_name);
            ...
            $LiveQueue = new Live_Queue($this->con, $merchant_name, $merchant_id);
            $LiveQueue->initialiseArray();

            $gps_data = array();

            array_push($gps_data, $longitude);
            array_push($gps_data, $latitude);
            array_push($gps_data, $merchant_id);

            $gps_data = json_encode($gps_data);

            $slide_data = array();

            array_push($slide_data, $merchant_id);

            $slide_data = json_encode($slide_data);
            ...
            $str .=
                  ...
                  <div class=merchant_$merchant_id>
                    ...
                    <div class='merchant_tabs'>
                      ...
                      <div class='templateBOOK' id='share_$merchant_id' onclick='openMerchantBooking($merchantUser_data)'>
                        Reserve
                      </div>
                    ...
                    <div class='merchant_title'>
                      ...
                      <input type='submit' class='location_button_$merchant_id' value='".$distance." km away with ".$waitingTime." minute wait' onclick='showMerchantWindow($gps_data)' style='text-decoration: none' id='locationButton'>
                      </div>

                      <br>

                      <div class='liveQueue' id='live_queue_$merchant_id'>".$LiveQueue->animate()."</div>
                      ...
                      <img src=$profile_pic width='640px' height='360px' id='merchant_photos_$merchant_id'>
                        <div id='merchant_messages_window' class='merchant_window_$merchant_id' style='height:0px; border:none;'></div>
                          ...
                          <div class='gallery_control_container' id='gallery_controller_$merchant_id'>
                            <a class='prev' onclick='getPreviousSlide($slide_data)'>&#10094;</a>
                            <a class='next' onclick='getNextSlide($slide_data)'>&#10095;</a>
                          </div>
                      </div>
